...
function EditCabinForm({ cabinToEdit = {}, setShowForm }) {
    const {id: editId, image: oldImage, ...editValues} = cabinToEdit;

    const { register, handleSubmit, reset, getValues, formState } = useForm({defaultValues: editValues}); // coming from 'react-hook-form'

    const queryClient = useQueryClient();

    const { isLoading: isCreating, mutate } = useMutation({
        mutationFn: ({newCabin, id, oldImg}) => editCabin(newCabin, id, oldImg),
        onSuccess: () => {
    ...
    function myOwnSubmitFn(data) { // onSubmit will trigger handleSubmit (provided by react-hook-form), which in tern will call 'myOwnSubmitFn'
        ...
        // As unlike createCabin, image field is not mandatory, so there could be 2 cases:
        // Case1: User do not updated image
        if(typeof (data.image) === 'string'){
            mutate({newCabin: data, id: editId, oldImg: ""});
        }
        // Case2: User updated image
        if(typeof (data.image) === 'object'){
            mutate({newCabin: {...data, image: data.image[0]}, id: editId, oldImg: oldImage});
        }
    }
    ...
    return (
        ...
        <Form onSubmit={handleSubmit(myOwnSubmitFn, myOwnError)}>
            ...
            <FormRow>
                ...
                })}  defaultValue={0} disabled={isCreating} />
